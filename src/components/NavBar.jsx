  ...
  useEffect(() => {
    if (searchInput) {
      const fetchMovies = fetch(
        ...
        .then((data) =>
          data.results.map((movie) => ({ ...movie, media_type: "movie" })),
        );

      const fetchTVShows = fetch(
        `https://api.themoviedb.org/3/search/tv?query=${searchInput}&include_adult=false&language=en-US&page=1`,
        options,
      )
        .then((response) => response.json())
        .then((data) =>
          data.results.map((tvShow) => ({ ...tvShow, media_type: "tv_show" })),
        );
      ...
      <header className="absolute left-0 top-0 w-full flex items-center h-24 z-40 mx-auto">
        <nav className="relative mx-auto lg:max-w-7xl w-full px-5 sm:px-10 md:px-12 lg:px-5 flex gap-x-5 justify-between items-center">
          <div className="flex items-center min-w-max relative">
            <a href="/" className="font-semibold flex items-center gap-x-2">
              <div className="flex items-center -space-x-3">
                <img
                  src="/logo.svg"
                  className="h-12 rounded-full dark:invert duration-300 ease-linear"
                  alt="brand"
                />
              </div>
            </a>
          ...
          <div
            ...
            `}
          >
            <ul className="flex flex-col justify-center lg:flex-row gap-8 lg:items-center text-gray-700 dark:text-gray-300 lg:w-full lg:pl-36">
              ...
              {searchResults.length > 0 && (
                <ul className="absolute w-full bg-white dark:bg-black border rounded-lg mt-2 transition duration-500">
                  {searchResults.slice(0, 5).map((result) => (
                    <Link
                      key={result.id}
                      to={`/${result.media_type === "movie" ? "movie" : "tv-show"}/${result.id}`}
                    >
                      <li className="p-2 text-black dark:text-gray-200 hover:bg-gray-200 dark:hover:bg-gray-950 flex items-center gap-2">
                        {result.poster_path ? (
                          <img
                          ...
                          <div className="flex items-center justify-center bg-gray-300 dark:bg-gray-700 flex-shrink-0 relative overflow-hidden w-10 h-14">
                            <svg
                        ...
                        <div>
                          <div>
                            {result.title || result.name
                              ? (result.title || result.name).length > 20
                                ? (result.title || result.name).substring(
                                    0,
                                    20,
                                  ) + "..."
                                : result.title || result.name
